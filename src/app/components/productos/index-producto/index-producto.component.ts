...
export class IndexProductoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._route.params.subscribe(
      params => {
        this.route_categoria = params['id'];
        ...
        if (this.route_categoria) {
          ...
          this._guestService.listar_productos_publico().subscribe(
            response => {
              ...
              console.log('PRODUCTOS CONST');
              this.productos_const = this.productos;

              console.log(this.productos_const);
              console.log('CATEGORIAS');
              console.log(this.categorias);
              let categoria = this.categorias.filter(item => item._id == this.route_categoria);
              ...
              this._guestService.listar_productos_filtro_publico(categoria[0]._id).subscribe(
                response => {
  ...
  buscar_por_categoria(cat: any) {
    if (cat == 'todos') {
      ...
      this._guestService.listar_productos_filtro_publico(cat).subscribe(
        response => {
          this.productos = response.data;
        }
      );
